...
class SDXLBase10:
    """
    Stable Diffusion XL Base 1.0 (Diffusers) wrapper.

    Notes:
    - Uses DiffusionPipeline (base only; no refiner).
    - Default model_dir points to your local folder.
    - By default uses CUDA if available, otherwise CPU.
    """

    model_dir: Union[str, Path] = DEFAULT_SDXL_BASE_DIR
    device: Optional[str] = None  # e.g. "cuda", "cuda:0", "cpu"
    torch_dtype: Optional[str] = None  # "float16" | "bfloat16" | "float32" | None (auto)
    use_safetensors: bool = True
    variant: Optional[str] = "fp16"  # SDXL commonly provides "fp16" weights

    _pipe: object = None

    def load(self):
        """
        Lazily load the DiffusionPipeline.
        ...
        # Only use variant when it makes sense (fp16 weights with float16 dtype).
        variant = self.variant
        if not (variant and dtype == torch.float16):
            variant = None
...
def get_sdxl_base(
    ...
    *,
    ...
    """
    Get a cached, loaded SDXL base instance to avoid re-loading weights.
