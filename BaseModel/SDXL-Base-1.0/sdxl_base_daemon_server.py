"""
SDXL Base 1.0 local daemon server (keep model alive).

Protocol (JSON Lines):
  Request:
    - {"cmd":"ping"}
    - {"cmd":"generate","prompt":"...","out":"/abs/path.png","steps":40,"guidance":7.5,
       "seed":123,"negative_prompt":"...","height":1024,"width":1024,"num_images_per_prompt":1}
  Response:
    - {"ok": true, "result": {...}}
...
class SDXLRequestHandler(socketserver.StreamRequestHandler):
    model = None
...
def main() -> None:
    ...
    SDXLRequestHandler.model = model

    with socketserver.TCPServer((args.host, args.port), SDXLRequestHandler) as srv:
        srv.allow_reuse_address = True
        print(
            f"SDXL Base 1.0 daemon listening on {args.host}:{args.port} "
